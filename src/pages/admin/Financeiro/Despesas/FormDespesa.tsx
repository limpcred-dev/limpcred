...
import { centroCustosService, despesasService } from '../../../../services/financeiro';
import { CentroCusto, DespesaCreate } from '../../../../types/financeiro';
...
export default function FormDespesa({ onClose, onSuccess }: FormDespesaProps) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    const carregarCentrosCusto = async () => {
      ...
      try {
        ...
        const centrosDespesa = lista.filter(c => 
          c.tipo === 'despesa' && 
          c.status === 'ativo'
        );
        setCentrosCusto(centrosDespesa);
      } catch (error) {
